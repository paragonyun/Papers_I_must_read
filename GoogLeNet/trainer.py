...
class Train :
    ...
    def train(self) :
        ...
        for imgs, labels in tqdm(self.tr_loader) :
            ...
            goo_preds, aux1_preds, aux2_preds = self.model(imgs)


            goo_loss = self.criterion(goo_preds, labels)
            aux1_loss = self.criterion(aux1_preds, labels)
            aux2_loss = self.criterion(aux2_preds, labels)

            loss = goo_loss + 0.3*aux1_loss + 0.3*aux2_loss

            acc = calculate_acc(goo_preds, labels)
